	...
	const DJPEG2KCodecParameter *djcp) const
{
	...
	if (result.good())
	{
		...
		opj_set_info_handler(l_codec, msg_callback_info, NULL);
		opj_set_warning_handler(l_codec, msg_callback_warn, NULL);
		opj_set_error_handler(l_codec, msg_callback_error, NULL);
	...
	const FMJPEG2KRepresentationParameter *djrp) const
{
	...
	switch(dinter->getRepresentation())
	{
		...
		case EPR_Sint16:
			...
			break;
		default:
			// unreachable: depth is restricted to <= 16 bits above
			break;
	}
