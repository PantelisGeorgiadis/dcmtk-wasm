...
OPJ_SIZE_T opj_read_from_memory(void* p_buffer, OPJ_SIZE_T p_nb_bytes,
                                DecodeData* p_file);
OPJ_SIZE_T opj_write_to_memory(void* p_buffer, OPJ_SIZE_T p_nb_bytes,
                               DecodeData* p_file);
...
OPJ_SIZE_T opj_read_from_memory(void* p_buffer, OPJ_SIZE_T nb_bytes,
                                DecodeData* p_user_data) {
...
OPJ_SIZE_T opj_write_to_memory(void* p_buffer, OPJ_SIZE_T nb_bytes,
                               DecodeData* p_user_data) {
  if (!p_user_data || !p_user_data->src_data || p_user_data->src_size == 0) {
    return -1;
  }
...
OPJ_OFF_T opj_skip_from_memory(OPJ_OFF_T nb_bytes, DecodeData* p_user_data) {
  ...
  // check overflow
  if (/* (nb_bytes > 0) && */ (
      p_user_data->offset > std::numeric_limits<OPJ_SIZE_T>::max() - nb_bytes))
    return -1;
...
opj_stream_t* OPJ_CALLCONV opj_stream_create_memory_stream(
    DecodeData* p_mem, OPJ_UINT32 p_size, bool p_is_read_stream) {
  ...
  opj_stream_set_read_function(l_stream,
                               (opj_stream_read_fn)opj_read_from_memory);
  opj_stream_set_write_function(l_stream,
                                (opj_stream_write_fn)opj_write_to_memory);
  opj_stream_set_skip_function(l_stream,
                               (opj_stream_skip_fn)opj_skip_from_memory);
  opj_stream_set_seek_function(l_stream,
                               (opj_stream_seek_fn)opj_seek_from_memory);
  return l_stream;
}

void msg_callback_info(char const* msg, void* f) {
  OutputDcmtkInfo("memory_file::msg_callback_info::" + std::string(msg));
}

void msg_callback_warn(char const* msg, void* f) {
  OutputDcmtkInfo("memory_file::msg_callback_warn::" + std::string(msg));
}

void msg_callback_error(char const* msg, void* f) {
  ThrowDcmtkException("memory_file::msg_callback_error::" + std::string(msg));
}
