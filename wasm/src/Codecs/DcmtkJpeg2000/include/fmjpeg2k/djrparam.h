 ...
 */
class FMJPEG2K_EXPORT FMJPEG2KRepresentationParameter
    : public DcmRepresentationParameter {
 public:
  /** constructor
   *  @param nearlosslessDeviation used as parameter NEAR in JPEG 2000
   * nearlossless-encoding process
   *  @param losslessProcess true if lossless process is requested
   */
  FMJPEG2KRepresentationParameter(Uint16 PSNR = 0,
                                  OFBool losslessProcess = OFTrue);

  /// copy constructor
  FMJPEG2KRepresentationParameter(FMJPEG2KRepresentationParameter const &arg);
